...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Slew antennas across a calibrator source.')
    parser.add_argument('--home', type=float,
                        help='the elevation to slew across')
    ...
    parser.add_argument('--refants', type=list)
    args = parser.parse_args()
    current_el = report_commanded_elevation()
    if np.abs(current_el-args.home) > args.range*2.1:
        print(f'Current elevation of {current_el} more than {2.1*args.range} from source elevation of {args.home}.  Please move antennas and rerun script.')
    else:
        slew_across(args.home, args.refants, range_el=args.range, step_el=args.step)
